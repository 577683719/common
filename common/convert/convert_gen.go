...
{{- range .Methods }}
 {{ $method := . }}
func {{.MethodName}}(source {{.SourceOldType}}) {{.TargetType}} {
	
 	conver:=&{{.RetruenName}}{
		{{- range .Fields }}
		{{- if not (shouldIgnore $method.IgnoreFields .SourceField) }}
		{{- if not (typeConver $method.TypeToStringFileds .SourceField) }}
		{{- if not (stringToTime $method.StringToTimeFields .SourceField) }}
		{{- if not (stringToDecimal $method.StringToDecimalFileds .SourceField) }}
	...
	{{- range .Fields }}
	{{- if (typeConver $method.TypeToStringFileds .SourceField) }}
	conver.{{.TargetField}}= convertFieldToString(source.{{.SourceField}})
	{{- end }}
	{{- if  (stringToTime $method.StringToTimeFields .SourceField) }}
	conver.{{.TargetField}}=  parseTime(source.{{.SourceField}})
	{{- end }}
	{{- if  (stringToDecimal $method.StringToDecimalFileds .SourceField) }}
	conver.{{.TargetField}}=  stringToDecimal(source.{{.SourceField}})
	{{- end }}
	{{- end }}

	return conver
}
func {{.MethodListName}}(source []{{.SourceOldType}}) []{{.TargetType}} {
	sourceList := []{{.TargetType}}{}
	for i := 0; i < len(source); i++ {
		sourceList = append(sourceList,{{.MethodName}}(source[i]))
	}
	return sourceList
}
{{- end }}
...
{{- range .Methods }}
 {{ $method := . }}
func {{.MethodName}}Pointer(source {{.SourceType}}) {{.TargetType}} {
	if source == nil {
		return &{{.RetruenName}}{};
	}
 	conver:=&{{.RetruenName}}{
...
func {{.MethodListName}}Pointer(source []{{.SourceType}}) []{{.TargetType}} {
	sourceList := []{{.TargetType}}{}
	for i := 0; i < len(source); i++ {
		sourceList = append(sourceList,{{.MethodName}}Pointer(source[i]))
	}
	return sourceList
}
...
func convertFieldToString(value interface{}) string {
	// 根据需要进行类型转换的逻辑
	switch v := value.(type) {
	case time.Time:
		if v.IsZero() {
		...
		return date_util.DateToDateTimeStr(v)
...
func stringToDecimal(str string) decimal.Decimal {
	fromString, _ := decimal.NewFromString(str)
	return fromString
}
`
...
func generateFields(source interface{}, target interface{}) ([]Field, error) {
	...
	for i := 0; i < targetType.NumField(); i++ {
		...
		for sourceName, sourceField := range sourceFieldMap {
			if strings.EqualFold(targetField.Name, sourceField.Name) { // Match by type for simplicity
				field := Field{
