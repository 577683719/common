...
{{- range .Methods }}
 {{ $method := . }}
func GetOmit(source {{.SourceOldType}}) []string {
	var str []string

	{{- range .Fields }}
	if isAnInvalidValue(source.{{.SourceField}}) {
		str =append(str, {{.FieldClo}} )
	}
	{{- end }}
...
{{- range .Methods }}
 {{ $method := . }}
func GetPointerOmit(source *{{.SourceOldType}}) []string {
	var str []string
...
func isAnInvalidValue(value interface{}) bool {
	switch v := value.(type) {
	case time.Time:
